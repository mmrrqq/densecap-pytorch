...
def get_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Train whole network for the first epoch half, only region view prediction for the second half.",
    )
    parser.add_argument(
        "--config-path",
        default="model_params",
        help="Path of a folder containing the models config.json file",
    )
    parser.add_argument("--epochs", default=3, help="Max epochs to train.")
    parser.add_argument(
        "--lookup-tables-path",
        default="./data/VG-regions-dicts-lite.pkl",
        help="Path to the pickled look up tables for tokenization created while preprocessing VG.",
    )
    parser.add_argument(
        "--losses",
        nargs="+",
        default=["v", "mv", "vc", "dcs", "mv_dcs", "cvp"],
        help="Specify the losses to be included for training. Default value contains all possible losses.",
    )
    parser.add_argument(
        "--model-name",
        help="Name of the model checkpoint file (excluding '.pth.tar')",
    )
    parser.add_argument(
        "--model-prefix",
        default=str(time.time()).split(".")[0],
        help="Prefix used for model checkpoint saves. Defaults to current timestamp.",
    )
    parser.add_argument(
        "--params-path",
        default="model_params",
    ...
    parser.add_argument(
        "--snare-annotations-path",
        default="../snare/amt/folds_adversarial",
        help="Path to the SNARE annotation files.",
    )
    parser.add_argument(
        "--snare-screenshots-path",
        default="../snare/data/screenshots",
        help="Path to the SNARE/ShapeNetSem model screenshots.",
    )
    parser.add_argument(
        "--test-categories",
        action="store_true",
        default=False,
        help="Compare the models performance on SNARE object categories.",
    )
    parser.add_argument(
        "--test-view-iterations",
        default=10,
        help="Iterations to test when '--test-view' is specified.",
    )
    parser.add_argument(
        "--test-view",
        action="store_true",
        default=False,
        help="Test random vs. predicted vantage point performance.",
    )
    parser.add_argument(
        "--train",
        action="store_true",
        default=False,
        help="Train/Finetune the model on the SNARE training dataset",
    )
...
def test_model_categories(
    model: DenseCapModel, data_loader: DataLoader
) -> Dict[str, float]:
    ...
    category_dict = build_category_dict(
        metadata_path="./data/shapenet_sem_metadata.csv"
    )
...
def test(model: DenseCapModel, data_loader: DataLoader) -> Dict[str, float]:
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader):
            ...
            cap_view_pred_correct += (
                losses1["cap_min_view"] == losses1["view_cap_preds"]
            ).sum()
...
def view_predict(model: DenseCapModel, images, query: str, best_view_id: int):
    ...
    _, best_view_losses, (best_view_caption, _) = model.query_caption(
        [pred_best_img], [query], torch.tensor([best_view_id])
    )
    _, random_view_losses, (random_view_caption, _) = model.query_caption(
        [rnd_img], [query], torch.tensor([rnd_view_id])
    )

    return (
        best_view_losses,
        random_view_losses,
        (rnd_view_id, random_view_caption, best_view_caption),
    )
        ...
        for _ in range(iterations):
            ...
            for i, batch in enumerate(tqdm(data_loader)):
                ...
                (
                    best_view_losses1,
                    random_losses1,
                    (rnd_view_id1, rnd_view_caption1, best_view_caption1),
                ) = view_predict(model, key1_imgs, annotation, best_view_id)
                ...
                (
                    best_view_losses2,
                    random_losses2,
                    (rnd_view_id2, rnd_view_caption2, best_view_caption2),
                ) = view_predict(model, key2_imgs, annotation, best_view_id)
...
def fine_tune_loop(args):
    ...
    test_set = SnareDataset(
        args.snare_annotations_path,
        args.snare_screenshots_path,
        mode="valid",
        filter_visual=True,
    )    

    train_set = SnareDataset(
        args.snare_annotations_path,
        args.snare_screenshots_path,
        mode="train",
        filter_visual=True,
    ...
    for epoch in range(args.epochs):
        ...
        for k, v in acc_dict.items():
            if v > best_acc_dict.get(k, -torch.inf):
                best_acc_dict[k] = v
                save_model(
                    model, None, None, v, iter_count, prefix=f"{args.model_prefix}_{k}"
                )
...
def eval_model(args):
    ...
    if args.test_view:
        ...
        print(
            ",".join(
                [
                    str((v.item() if torch.is_tensor(v) else v))
